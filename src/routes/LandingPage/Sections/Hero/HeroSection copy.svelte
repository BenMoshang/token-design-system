<script lang="ts">
	// Provide constant hero text; note the updated spelling for 'DEVELOPMENT'
	// import BenefitCard from './components/BenefitCard.svelte';
	const heroText = {
		title: 'MODERN WEB\nDEVELOPMENT',
	...
	<ul class="hero__benefits">
		...
		<li class="util__stack hero__office-location-wrapper">
			<svg xmlns="http://www.w3.org/2000/svg" height="10" width="7.5" viewBox="0 0 384 512"
				...
				/></svg
			>
			<p class="util__benefit--dark">BASED IN,</p>
			<p class="util__benefit--light hero__office-location">{heroText.officeLocation}</p>
		</li>
		...
		<li class="util__stack">
			...
			<p class="util__benefit--dark">AVAILABLE,</p>
			<p class="util__benefit--light">{heroText.available}</p>
		</li>
		...
		<li class="util__stack">
			...
			<p class="util__benefit--dark">WEB DEVELOPMENT</p>
			<p class="util__benefit--light">{heroText.Work}</p>
		</li>
	</ul>
</section>
...
<style lang="scss">
	.hero {
		...
		/*----icon variables----*/
		--icon-size: 1.5rem;
		--icon-margin-bottom: #{get-static-sp('s8')};
	...
	.hero {
		...
		&__divider {
			...
			inline-size: 100%;
			block-size: 0.125rem;
			margin: get-static-sp('s4') 0 get-static-sp('s64');
			opacity: 0.5;
